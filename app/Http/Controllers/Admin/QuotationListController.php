...
class QuotationListController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data->where('quotations.branch', session('branch_code'));
    ...
    public function download_excel(Request $request)
    {
        try 
        {
            ...
            $data = QuotationItem::join('quotations', 'quotation_items.quotation_id', '=', 'quotations.id')->join('products', 'quotation_items.itemCode', '=', 'products.productCode')->join('customers', 'quotations.CustomerCode', '=', 'customers.customer_code')->join('partners', 'quotations.Ref3', '=', 'partners.partner_code') ->select(
                'quotations.DocNo', 
                'quotations.status', // Specify the table for status to avoid ambiguity
                'quotations.DocDate', 
                'quotation_items.ItemCode', 
                'products.productName', 
                'products.brand', 
                'quotation_items.Qty', 
                'quotation_items.LineTotal', 
