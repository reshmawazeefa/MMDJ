...
class OutgoingPaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            if (Auth::user()->hasRole('Admin')) {
                $data = OutgoingPaymentMaster::select('outgoing_payment_masters.*')->with(array('customer', 'referral1', 'referral2', 'referral3','Item_details','Item_details2','user'));
            } else {
                $data = OutgoingPaymentMaster::select('outgoing_payment_masters.*')->with(array('customer', 'referral1', 'referral2', 'referral3','Item_details','Item_details2','user'))
            ...
            if (!empty($request->customer)) {
                //echo "here";
                $data->where('outgoing_payment_masters.CardCode', $request->customer);
            }
            if (!empty($request->user)) {
                //echo "here";
                $data->where('outgoing_payment_masters.SalesManCode', $request->user);
            }
            if (!empty($request->status)) {
                if ($request->status == "All") {
                    $data->where('outgoing_payment_masters.IntgrStatus', '!=', 'Cancel');
                } else {
                    $data->where('outgoing_payment_masters.IntgrStatus', $request->status);
                }
            } else {
                $data->where('outgoing_payment_masters.IntgrStatus', 'Open');
            }
                $data->where('outgoing_payment_masters.Branch', session('branch_code'));
                ...
                ->addColumn('action', function ($row) {
                    $url = url('admin/outgoing-payment/' . $row->DocEntry);
        ...
        return view('admin.outgoingpayments');
    }
    ...
    public function create()
    {
        ...
        ->get();
        return view('admin.create_outgoingpayment', compact('accmaster'));
    }
    ...
    public function get_accounts_payment(Request $request)//get Products
    {
        $data = [];
        $page = $request->page;
        $resultCount = 25;
        ...
        if ($request->has('q') && $request->q!= '') {
            ...
            ->select("AcctCode","AcctName")
            ->where(function ($query) use ($search) {
                $query->Where('AcctCode','=',"130030108");
            })->skip($offset)->take($resultCount)->get();
        ...
        $count =DB::table('account_masters')
        ->select("AcctCode","AcctName")->where(function ($query){
            $query->Where('AcctCode','=',"130030108");
        })->count();
        }
        /**set pagination**/
    ...
    public function show($id)
    {
        ...
        $details1 = OutgoingPaymentMaster::select('outgoing_payment_masters.*')->with(array('Item_details', 'Item_details2','customer', 'referral3','user'))->where('outgoing_payment_masters.DocEntry',$id)->get(); 
        //  dd($details1);
        return view('admin.outgoingpayment_details', compact('details1'));
    }
    ...
    public function get_customers(Request $request)//get Customers
    {
        ...
        if ($request->has('q') && $request->q!= '') {
            ...
            ->where('purchase_invoice_masters.status', "Open")
        ...
        // $count =PurchaseInvoiceMaster::with('customer')->select("customers.id","customers.phone","customers.name")->where('customer.is_active',1)->count();
    ...
    public function list_customers(Request $request)//get Customers
    {
        ...
        if ($request->has('q') && $request->q!= '') {
            $search = $request->q;
            $data = OutgoingPaymentMaster::join('customers', 'outgoing_payment_masters.CardCode', '=', 'customers.customer_code')
            ->select('customers.id','customers.phone','customers.customer_code', 'customers.name')
            ->where('customer.is_active',1)
            ->where('outgoing_payment_masters.Branch', session('branch_code'))
            ->where(function($query) use ($search){$query->where('customers.name','LIKE',"%$search%")
            ->orWhere('customers.phone','LIKE',"%$search%");
            })            
            ->groupBy('customers.id','customers.phone','customers.customer_code', 'customers.name')
            ...
            $count = OutgoingPaymentMaster::join('customers', 'outgoing_payment_masters.CardCode', '=', 'customers.customer_code')
            ->select('customers.id','customers.phone','customers.customer_code', 'customers.name')
            ->where(function($query) use ($search){$query->where('customers.name','LIKE',"%$search%")
            ->orWhere('customers.phone','LIKE',"%$search%");
            })            
    ...
    public function get_cus(Request $request)//get Customers
    {
        $data = [];
        $page = $request->page;
        $resultCount = 25;
        ...
        if ($request->has('q') && $request->q!= '') {
            $search = $request->q;
            $data = SalesReturnMaster::join('customers', 'sales_return_masters.cid', '=', 'customers.customer_code')
            ->select('customers.id','customers.phone','customers.customer_code', 'customers.name')
            ->where('customers.is_active',1)
            ->where('sales_return_masters.branch', session('branch_code'))
            ->where('sales_return_masters.status', "Open")
            ->where(function($query) use ($search){$query->where('customers.name','LIKE',"%$search%")
            ->orWhere('customers.phone','LIKE',"%$search%");
            })            
            ->groupBy('customers.id','customers.phone','customers.customer_code', 'customers.name')
            ...
            $count = SalesReturnMaster::join('customers', 'sales_return_masters.cid', '=', 'customers.customer_code')
            ->select('customers.id','customers.phone','customers.customer_code', 'customers.name')
            ->where(function($query) use ($search){$query->where('customers.name','LIKE',"%$search%")
            ->orWhere('customers.phone','LIKE',"%$search%");
            })            
        ...
        // $count =SalesReturnMaster::with('customer')->select("customers.id","customers.phone","customers.name")->where('customer.is_active',1)->count();
    ...
    public function insert(Request $request)
        {
            ...
            if($request->account_type != 'account')
            {
                ...
                elseif($request->account_type == 'supplier' || $request->account_type == 'advance')
                    {
                ...
                $docnum = $request->doc_list . "-" . $request->docNumber;
                $outgoing_master = OutgoingPaymentMaster::create([
                    'Series' => null,
                    'DocNum' => $docnum,
                    ...
                    'CreateDate' => $request->posting_date,
                    'DocTime' => null,
                            ...
                            if(isset($request->rows))
                            {
                                foreach ($request->rows as $row) {
                                    if (isset($row['checked']) && $row['checked'] == '1') {
                                        ...
                                        DB::table('outgoingpayments_lines')->insert([
                                            'LineNum' => $master_id,
                                            'BaseType' => 13,
                                            'BaseEntry' => null,
                                            'InvDocNum' =>$row['doc_num'],
                                            'InvSeries' => null,
                                            'InvDocDate' => $row['doc_date'],
                                            'InvDocTotal' => $row['total'],
                                            'SumApplied' => $row['sum_applied'],
                                            'LineStatus' => null,
                                            'CostCenter1' => $row['cost_centre'],
                                            'CostCenter2' => null,
                                            'CostCenter3' => null,
                                            'CostCenter4' => null,
                                            'CostCenter5' => null,
                                        ...
                                        // DB::table('sales_return_masters as i')
                                        //     ->where('i.doc_num', $row['doc_num'])
                                        //     ->update([
                                        //         'applied_amount' => DB::raw("applied_amount + $sumapp")
                                        //     ]);


                                        $newAppliedAmount = DB::table('sales_return_masters')
                            ...
                            if(isset($request->rows))
                            {
                                foreach ($request->rows as $row) {
                                    if (isset($row['checked']) && $row['checked'] == '1') {
                                        ...
                                        // DB::table('purchase_invoice_masters as i')
                                        //     ->where('i.doc_num', $row['doc_num'])
                                        //     ->update([
                                        //         'applied_amount' => DB::raw("applied_amount + $sumapp")
                                        //     ]);
                                        ...
                                        $newAppliedAmount = DB::table('purchase_invoice_masters')
                                        ->where('doc_num', $row['doc_num'])
                                        ->value(DB::raw("ROUND(applied_amount + $sumapp, 2)"));
                 ...
                 if($request->account_type == 'account')
                {
                    ...
                    $grand_total = 0;
                        ...
                        for ($i = 0; $i < $count; $i++) 
                        {
                            if (!empty($accountlist[$i]) && !empty($acc_code[$i])) 
                            {
                                ...
                                DB::table('outgoingpayments_lines2')->insert([
                    ...
                    session()->flash('success', 'Outgoing payment added successfully');
                    return response()->json([
                        'success' => true,
                        'outgoingpayment_id' => $master_id, 
                        'message' => 'Outgoing payment added successfully'
                    ]);
        ...
        public function download_excel(Request $request)
        {
            try 
            {
                $data = DB::table('outgoing_payment_masters')
                    ->join('customers', 'outgoing_payment_masters.CardCode', '=', 'customers.customer_code')
                    ->join('partners', 'outgoing_payment_masters.SalesManCode', '=', 'partners.partner_code')
                    ->select(
                        'outgoing_payment_masters.DocNum',
                        'outgoing_payment_masters.IntgrStatus',
                        'outgoing_payment_masters.DocDate',
                        'outgoing_payment_masters.TotalPaySum',
                ...
                if (!empty($request->from_date)) {
                    $data->where('outgoing_payment_masters.DocDate', '>=', $request->from_date);
                }
                if (!empty($request->to_date)) {
                    $data->where('outgoing_payment_masters.DocDate', '<=', $request->to_date);
                }
                if (!empty($request->customer)) {
                    $data->where('outgoing_payment_masters.CardCode', $request->customer);
                }
                ...
                if (!empty($request->status)) {
                    if ($request->status == "All") {
                        $data->whereIn('outgoing_payment_masters.IntgrStatus', $request->stsval);
                    } else {
                        $data->where('outgoing_payment_masters.IntgrStatus', $request->status);
                    }
                } else {
                ...
                $file_name = 'outgoing-payment-' . time() . '.xlsx';
    ...
    public function get_cus_details(Request $request)
    {
        if(isset($request['customer_code']))
        {
            ...
            // Fetch the quotations related to the customer
            $purchaseinvoice = DB::table('sales_return_masters as q')
                            // ->join('sales_return_items as qt', 'q.id', '=', 'qt.sm_id')
                            ->where('q.status', 'Open')
                            ->where('q.cid', $request['customer_code'])
                            ->where('q.branch', session('branch_code'))
                            ->select('q.doc_num','q.doc_date','q.total','q.applied_amount')
    ...
    public function outgoing_get_account_code(Request $request)
    {
