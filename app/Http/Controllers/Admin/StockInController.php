    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (Auth::user()->hasRole('Admin')) {
                $data = StockInMaster::select('stock_in_masters.*')->with(array('Branch', 'referral1', 'referral2', 'referral3'))->where('stock_in_masters.to_branch', session('branch_code'));
            } else {
                $data = StockInMaster::select('stock_in_masters.*')->with(array('Branch', 'referral1', 'referral2', 'referral3'))->where('stock_in_masters.to_branch', session('branch_code'))
            ...
            if (!empty($request->branch)) {
                //echo "here";
                $data->where('stock_in_masters.from_branch', $request->branch);
            }
           
            if (!empty($request->user)) {
                //echo "here";
                $data->where('stock_in_masters.emp_id', $request->user);
            }
            if (!empty($request->status)) {
                if ($request->status == "All") {
                    $data->where('stock_in_masters.status', '!=', 'Cancel');
                } else {
                    $data->where('stock_in_masters.status', $request->status);
                }
            } else {
                $data->where('stock_in_masters.status', 'Open');
            }
                ...
                ->addColumn('action', function ($row) {
                    $url = url('admin/stockin/' . $row->id);
                    $url_edit = url('admin/stockin/' . $row->id . '/edit');
                ...
                ->addColumn('from_branch_name', function ($row) {
                    return $row->from_branch_name ?? 'N/A';
                })
...
// print_r($queries);
    ...
    public function create()
    {
        ...
        // dd($branch_address);
        return view('admin.create_stockin', ['branch' => $branch, 'address_details' => $branch_address]);
    ...
    public function get_stock_details(Request $request)
    {
        if(isset($request['branch_code']))
        {
            ...
            $data = StockOutMaster::where('from_branch', $request['branch_code'])
                            ->where('status', 'open')
                            ->where('to_branch',session('branch_code'))
                            ->first();

            // Fetch the quotations related to the customer
            $stock_out = DB::table('stock_out_masters as s')
            ->join('stock_out_items as st', 's.id', '=', 'st.sm_id')
            ->where('s.status', 'Open')
            // ->where('st.qty', '>', 0)
            ->where('s.from_branch', $request['branch_code'])
            ->where('s.to_branch',session('branch_code'))
...
// dd($stock_out);
    ...
    public function get_branch_open_stock(Request $request)
    {
        if(isset($request['branch_code']))
        {
            ...
            $stockout = DB::table('stock_out_masters as s')
            ->join('stock_out_items as st', 's.id', '=', 'st.sm_id')
            ->where('s.status', 'open') // Only open quotations
            ->where('s.from_branch', $request['branch_code']) // Customer-specific quotations
            ->where('s.to_branch',session('branch_code'))
            ->where(function ($query) {
    ...
    public function get_stockout_details(Request $request)
    {
        if ($request->ajax()) {

            $sorderId = $request['stock_out'];

            $stock_transfer = DB::table('stock_out_masters')
                ->join('stock_out_items', 'stock_out_masters.id', '=', 'stock_out_items.sm_id')
                ...
                ->whereIn('stock_out_masters.id', $sorderId) // equivalent to WHERE IN (4, 2)
                // ->where('stock_out_items.status', '=', 'Open')
                ->where(function ($query) {
                    $query->where('stock_out_items.status', '<>', 'Cancel')
                        ->where('stock_out_items.status', '<>', 'Confirm')
                        ->where('stock_out_items.qty', '<>', 0);
                ...
                ->select(
                    ...
                    'warehouses.whsName', 
                    'branches.*',
                    'stock_out_items.open_qty', 
                    'stock_out_items.unit_price', 
                    'stock_out_items.id', 
                    'stock_out_items.status',
                    'partners.name',
            ...
            $responseData = $stock_transfer->map(function($item) {
                // $line_total = round($item->line_total, 2);
                return [
                    'sales_emp_id' => $item->emp_id,
    ...
    public function insert(Request $request)
    {
       //dd($request);
        ...
        $stockin = new StockInMaster;
        $stockin->from_branch = $request->from_branch;
        $stockin->from_branch_name = $request->from_branch_name;
        $stockin->to_branch = $request->to_branch;
        $stockin->to_branch_name = $request->to_branch_name;
        $stockin->address_billto = $request->bill_to_address;
        $stockin->doc_number = $docnum;
        $stockin->open_stock = $request->open_qutn;
        $stockin->status = $request->status;
        $stockin->posting_date = $request->posting_date;
        $stockin->delivery_date = $request->delivery_date;
        $stockin->doc_date = $request->DocuDate;
        $stockin->emp_id = $request->partner3;
        $stockin->remarks = $request->remarks;
        $stockin->total_qty = $request->total_qty;
        $stockin->total_price = $request->total_price;
        $stockin->createdBy = Auth::user()->id;
        $stockin->updatedBy = null;
        $stockin->save();

        $stockin_id = $stockin->id;

        // if(isset($stockin->open_stock))
        // {
        //     $result = StockOutMaster::find($stockin->open_stock);
        //      if ($result) {
        //          $result->status = 'Close';
        //           $result->save();
        //      }
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (!empty($products[$i]) && !empty($quantity[$i]) && !empty($warehouses[$i])) {
                ...
                $bstock = DB::table('itemwarehouses as i')
                            ...
                            ->where('i.WhsCode', $request->to_branch)
                            ->first();
                ...
                if (isset($invoice_item_id[$i])) {
                    $quotationitem = StockOutItem::where('id', $invoice_item_id[$i])->first();
                ...
                $stockinItem->sm_id = $stockin_id;
                $stockinItem->item_id = $products[$i];
                $stockinItem->whscode = $warehouses[$i];
                $stockinItem->qty = $quantity[$i];
                $stockinItem->open_qty = $quantity[$i];
                $stockinItem->unit_price = $unitprice[$i];
                $stockinItem->serial_no = $serialno[$i];                
                $stockinItem->barcode = $barcode[$i];
                $stockinItem->status = "Open";
                $stockinItem->save();
            }
        ...
        session()->flash('success', 'Stock In added successfully');
        // return response()->json([
        //     'success' => true,
        //     'stockin_id' => $stockin_id,  // Pass the saved Sales Order ID
        //     'message' => 'Stock In added successfully'
        // ]);

        // return redirect('admin/stockin/' . $stockin_id)->with('success', 'Stock In added successfully');
        return redirect('admin/stock-in')->with('success', 'Stock In added successfully');
    }
    ...
    public function stockout_close_items(Request $request)
    {
        $id=$request['item'];
        $stockout_item = StockOutItem::find($id);
        $data = '';
        if ($stockout_item) {
            $stockout_item->status = 'Cancel';
            $stockout_item->save();
            $data = "Stock out item is cancelled!";
        }
        echo json_encode($data);
    }
    public function show($id)
    {
        $details = StockInMaster::select('stock_in_masters.*')->with(array('Item_details.products.stock.warehouse', 'customer', 'referral1', 'referral2','referral3'))->find($id); 
        //dd($details);
        return view('admin.stock_in_details', compact('details'));
    }

    public function close(Request $request, $id)
    {
       // dd($request);
        $stock_out = StockInMaster::find($id);
        $data = '';
        if ($stock_out->status == 'Confirm') {
            $data = "Stock In is already confirmed!";
        } else {
            $stock_out->status = 'Cancel';
            $stock_out->cancelReason = $request->cancel_reason;
            $stock_out->save();
            $data = "Stock In is cancelled!";
        }
        echo json_encode($data);
    }
    ...
    public function download_excel(Request $request)
    {
        ...
        try 
        {
            ...
            $data = StockInMaster::join('stock_in_items', 'stock_in_items.sm_id', '=', 'stock_in_masters.id')
            ->join('products', 'stock_in_items.item_id', '=', 'products.productCode')
            ->join('branches', 'stock_in_masters.to_branch', '=', 'branches.BranchCode')
            ->join('partners', 'stock_in_masters.emp_id', '=', 'partners.partner_code')
            ->where('stock_in_masters.to_branch', session('branch_code'))
            ->select('stock_in_masters.doc_number',
            'stock_in_masters.status',
            'stock_in_masters.doc_date',
            ...
            if (!empty($request->from_date)) {
                //echo "here1";
                $data->where('stock_in_masters.doc_date', '>=', $request->from_date);
            }
            if (!empty($request->to_date)) {
                //echo "here2";
                $data->where('stock_in_masters.doc_date', '<=', $request->to_date);
            }
            if (!empty($request->branch)) {
                //echo "here";
                $data->where('stock_in_masters.from_branch', $request->branch);
            ...
            if (!empty($request->status)) {
                if ($request->status == "All") {
                    ...
                    $data->whereIn('stock_in_masters.status',$request->stsval);
                } else {
                    $data->where('stock_in_masters.status', $request->status);
                }
            } else {
...
public function barcode_vefication_data(Request $request)
{
    // dd($request);
$data = [];
    
    // Get the form data (items) from the request
    $items = $request->input('items');

    // Loop through each item and prepare the data
    foreach ($items as $index => $item) {
        $data[] = [
            'DocEntry' => "",  // Assuming 'rnumber' is used as DocEntry
            'DocNum' => $request->input('rnumber'), // You can change or add logic for this field
            'LineNum' => $item['quotation_item_id'], // This assumes you want line numbers starting from 1
            'ItemCode' => $item['mproductcode'], // Barcode or other identifier, adjust as needed
            'ItemName' => $item['mproduct'],
            'ReceiptQty' => $item['mquantity'],
            'ScannedQty' => $item['msquantity'],
            'DifferenceQty' => $item['mdquantity'],
            'BaseEntry' => 0, // You might need to adjust this
            'created_at' => now(),
            'updated_at' => now(),
        ];
    }

    // Insert all items at once into the 'barcode_verification_details' table
    DB::table('barcode_verification_details')->insert($data);
