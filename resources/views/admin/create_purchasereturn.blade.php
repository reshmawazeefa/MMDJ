@extends('layouts.vertical', ["page_title"=> "Purchase Return Create"])
...
<div class="container-fluid">
                ...
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item">
                            <a href="{{url('admin/goods_receipt_po')}}">Purchase Return</a></li>
    ...
    <div class="row">
        <div class="col-12">@if(!empty($errors->all()))
            ...
            @endif
            <div class="card">
                <form id="purchase-return-submit" method="post" class="parsley-examples" action="{{url('admin/purchase-return/insert')}}">
                    {{csrf_field()}}
...
<div class="col-lg-4" >
    ...
    <div class="col-sm-12 mt-1">
        <a class="btn btn-primary waves-effect waves-light mb-0 qutt-bttn" id="copy_from_qutn" name="copy_from_qutn">Copy from Purchase Invoice</a>
        </div>
                        ...
                        <div class="ech-tr" id="tr_{{$i}}">
                            ...
                            <div class="echtr-inn">
                                <div class="row">
                                    ...
                                    <div class="colbor col-xl-6 col-lg-5 col-md-5 col-sm-5 col-10">
                                        <div class="ech-td">
                                            <label class="td-label">Remarks <span class="text-danger">*</span>
                                            
                                            </label>
                                            <div class="td-value">
                                            <!-- <input min="1" type="text"  id="remarks_{{$i}}" required class="remarks_{{$i}} form-control"  name="remarks[]"> -->
                                            <textarea id="remarks_{{$i}}" required class="remarks_{{$i}} form-control"  name="remarks[]"></textarea>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="colbor col-xl-5 col-lg-5 col-md-5 col-sm-5 col-10">
                                        <div class="ech-td">
                                            <label class="td-label">Total Amount <span class="text-danger">*</span></label>
                                            <div class="td-value">
                                            <input min="1" type="text"    id="linetotal_{{$i}}" required class="linetotal_{{$i}} form-control"  name="linetotal[]">
                                            </div>
                                        </div>
                                    </div>
...
<div class="row">
    <div class="col-sm-4"  style="display: none;">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-7">
            <textarea placeholder="Remarks" id="fremarks" type="text" name="fremarks" class="form-control"></textarea>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        $.ajax({
           type:'POST',
           url:"{{ url('admin/code') }}",
           data:{_token: "{{ csrf_token() }}",type:'purchasereturn'},
           success:function(data){
    ...
    // Trigger the change event on page load to set the default value
        ...
        $(".customerSelect").select2({
            ajax: {
            url: "{{ url('admin/ajax/pin_supplier') }}",
            type: 'POST',
                    dataType: 'json',
                    delay: 250,
...
function price_calc(r) {
    ...
    var amount_id = '.amount_' + r;

    var linetotal = parseFloat($(linetotal_id).val()) || 0;
    var amount = parseFloat($(amount_id).val()) || 0;

    if (linetotal !== 0) {
        if (linetotal > amount) {
            alert("This much amount is not available");
            $("#linetotal_" + r).val(amount);
        }
    }
    ...
    function calculate_footer()
     {
            ...
            let expense = parseFloat(rawValue.trim()) || 0; 
            ...
            $('.ech-tr').each(function(index) {
                ...
                {
                        let $row = $(this);
                        let linetotal = parseFloat($row.find('[name="linetotal[]"]').val()) || 0;
            ...
            // Set Grand Total
            $('#grand_total').val(totalLineDisc.toFixed(2));
...
function calculate_footerdiscount(val=0)
{
    ...
    $("#purchase-return-submit").submit(function(e){
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.success) {
                ...
                // window.location.href = 'purchase-return/' + data.puchasereturn_id;
...
$('#customer').change(function() {
    ...
    if (selectedValue) {
        $.ajax({
            url: "{{ url('admin/purchase-return/ajax/customer_details') }}",
            type: 'GET',
            ...
            success: function(response) {
                ...
                if (response) {
                    ...
                    var purchaseinvoice = response.purchaseinvoice;
        ...
        $("#copy_from_qutn").click(function(e) {
            ...
            $.ajax({
                url: "{{ url('admin/ajax/customer_open_purchase_invoice') }}",
                type: 'GET',
                data: { customer_code: selectedValue },
                ...
                success: function(data) { // Change 'quotation' to 'data' here
                    ...
                    $('#open_qutn').empty().append('<option disabled>Select a Purchase Invoice</option>');

                    // Check if data is not empty
                    if (data.length > 0) {
                        $.each(data, function(index, purchase_invoice) {
                            $('#open_qutn').append(
                                $('<option></option>').val(purchase_invoice.id).text(purchase_invoice.doc_num)
                            );
                        });
                    } else {
                        $('#open_qutn').append('<option value="">No purchase invoice available</option>');
                    }
                },
...
$('#open_qutn').change(function() {
    ...
    if (selectedValue) {
        $.ajax({
            url: "{{ route('admin.purchase_return.ajax.get_purchase_invoice_details') }}", // Update with your endpoint
            type: 'GET',
                            ...
                            <div class="ech-tr" id="tr_${i}">
                                ...
                                <div class="echtr-inn">
                                    <div class="row">
                                        ...
                                        <div class="colbor col-xl-6 col-lg-5 col-md-5 col-sm-5 col-10">
                                            <div class="ech-td">
                                                <label class="td-label">Remark(s)</label>
                                                ...
                                                <div class="td-value">
                                                    ...
                                                    <textarea class="form-control remarks_${i}" name="remarks[]" id="remarks_${i}" required>${item.remarks}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="colbor col-xl-5 col-lg-5 col-md-5 col-sm-5 col-10">
                                            <div class="ech-td">
                                                <label class="td-label">Total Amount</label>
                                                <div class="td-value">
                                                    <input type="number"  step="0.01"  id="linetotal_${i}" required class="linetotal_${i} form-control" value="${item.total_amount}" onKeyup="price_calc(${i})"  name="linetotal[]">
                                                    <input type="hidden" id="amount_${i}" required class="amount_${i} form-control" value="${item.total_amount}" name="amount[]">
                                                 </div>
                                            </div>
                                        </div>
...
$(document).on('click', '.remove-row', function(e) {
    ...
    // AJAX call for closing the Purchase Invoice item
    $.ajax({
