@extends('layouts.vertical', ["page_title"=> "Item History"])
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="javascript: void(0);">Item History</a></li>
                        <li class="breadcrumb-item active">Table</li>
                    </ol>
                </div>
                <h4 class="page-title">Item History</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="row mb-4 pb-2 dashed-borderStyle">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-4">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-7">
                                           <select id="products"  name="products" class="itemSelect form-control select2"></select>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-12">
                    ...
                    <div class="StyleS">
                    <div class="col-12">
                    <span class="me-3">Opening Stock: <span id="totalInQty"></span></span>
                    <span>Closing Stock: <span id="totalOutQty"></span></span> 
                    </div>
                    </div>
                      <!-- <span>Opening Stock: <span id="totalInQty"></span></span> -->
                        <thead>
...
<script type="text/javascript">
  $(document).ready(function() {
    $(".itemSelect").select2({
            ...
            var url = "{{ url('admin/item_history') }}";
            var url = "{{ url('admin/item_history') }}";
            var table = $('#server-datatable').DataTable({
                ...
                ajax: {
                    ...
                    data: function (d) {
                        ...
                        d.item = $('select[name="products"]').val()
                    },
                    dataSrc: function (json) {
                        console.log("Total InQty:", json.totalInQty);  // Log totalInQty from JSON response
                        console.log("Total OutQty:", json.totalOutQty); // Log totalOutQty from JSON response
                        if (json.totalInQty !== undefined && json.totalOutQty !== undefined) {
                            $('#totalInQty').text(json.totalInQty); // Display total InQty
                            $('#totalOutQty').text(json.totalOutQty); // Display total OutQty
                        }
                        return json.data; // Ensure only the data part is returned
                    },
                    initComplete: function (settings, json) {
                        console.log("Init complete", json);  // Check initComplete logs
                        if (json.totalInQty !== undefined && json.totalOutQty !== undefined) {
                            $('#totalInQty').text(json.totalInQty); // Display total InQty
                            $('#totalOutQty').text(json.totalOutQty); // Display total OutQty
                        }
                ...
                columns: [
                    ...
                    { data: 'ItemName', name: 'ItemName' },        // Index 1
                    { data: 'TransNo', name: 'TransNo' },          // Index 2
                    { data: 'TransType', name: 'TransType' },      // Index 3
                    { data: 'DocDate', name: 'DocDate' },          // Index 4
                    { data: 'CardName', name: 'CardName' },        // Index 5
                    { data: 'LineID', name: 'LineID' },            // Index 6
                    { data: 'InQty', name: 'InQty' },              // Index 7
                    { data: 'OutQty', name: 'OutQty' },            // Index 8
                    { data: 'WhsCode', name: 'WhsCode' }           // Index 9
                ],
                ...
                footerCallback: function (row, data, start, end, display) {
                    ...
                    // Calculate sum of 'InQty' column (column 7)
                    var totalInQty = api.column(7).data().reduce(function (a, b) {
                        return parseFloat(a) + parseFloat(b);
                    }, 0);

                    // Calculate sum of 'OutQty' column (column 8)
                    var totalOutQty = api.column(8).data().reduce(function (a, b) {
                        return parseFloat(a) + parseFloat(b);
                    }, 0);

                    // Round the sums to 2 decimal places
                    totalInQty = totalInQty.toFixed(2);
                    totalOutQty = totalOutQty.toFixed(2);

                    // Display the sums in the footer
                    $(api.column(7).footer()).html(totalInQty);
                    $(api.column(8).footer()).html(totalOutQty);
                }
            });
