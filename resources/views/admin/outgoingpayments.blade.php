@extends('layouts.vertical', ["page_title"=> "Outgoing Payment | List"])
...
<div class="container-fluid">
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="javascript: void(0);">Outgoing Payments</a></li>
                        <li class="breadcrumb-item active">Table</li>
                    </ol>
                </div>
                <h4 class="page-title">Outgoing Payments</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
    ...
    <div class="row">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body body-style productMStyle table-spcl2 without-button incoming-button">
                    <div class="row mb-0 pb-2 dashed-borderStyle">
                        ...
                        <div class="col-sm-8">
                            <div class="text-sm-end mt-2 mb-2 mt-sm-0">
                                <a href="{{url('admin/outgoing-payment-create')}}" class="btn btn-primary waves-effect waves-light"><i class="mdi mdi-plus-circle me-1"></i> Add Outgoing Payment</a>
                            </div>
                        </div>
                        <form id="sales-orderearchForm" class="form" method="POST" action="{{url('admin/outgoingpayment/excel')}}">
                            @csrf
                            <div class="col-lg-12">
                                <div class="row">
                                    <div class="col-lg-4">
                                    ...
                                    <div class="col-lg-4">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <input id="outgoing-payment-searchSubmit" class="btn btn-info" type="button" value="Search" /> 
                                            <input id="sales-orderearchReset" class="btn btn-info" type="button" value="Reset" /> 
                                            <!-- <input id="outgoing-paymentsearchExport" class="btn btn-info" type="button" value="Excel Export" /> -->
                                            </div>
                                        </div>  
                                    </div>
                                </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body-approve p-4">
                <div class="text-center">
                    <i class="dripicons-warning h1 text-warning"></i>
                    <h4 class="mt-2">Do you really want to confirm the outgoing Payment?</h4>
...
<script type="text/javascript">
    $(document).ready(function () {
        var url = "{{url('admin/outgoing-payment')}}";
        var table = $('#server-datatable').DataTable({
            ...
            columns: [
                ...
                { data: 'docdue_date', name: 'docdue_date', orderable: false },
        ...
        $(document).on('click', '#outgoing-payment-searchSubmit', function (e) {
            //e.preventDefault();
            console.log(url);
            table.ajax.url(url).load();
        });

        $(document).on("click", "#outgoing-paymentsearchExport", function () {
            $('#ExportDownload').hide();
            $('#outgoing-paymentsearchExport').val("Downloading..");
            $("#outgoing-paymentsearchExport").attr("disabled", true);
            var url = "{{url('admin/outgoingpayment/excel')}}";
            ...
            $.ajax({
                ...
                success: function (d) {
                    ...
                    $('#outgoing-paymentsearchExport').val("Excel Export");
                    $("#outgoing-paymentsearchExport").attr("disabled", false);
                    $('#ExportDownload').show();
       ...
       $(".userSelect").select2({
        ...
        $(".customerSelect").select2({
            ajax: {
                url: "{{ url('admin/ajax/list-outgoing-customers') }}",
                type: 'POST',
                dataType: 'json',
                delay: 250,
                data: function (params) {
    ...
    $(".modal-body .text-center button").on("click", function (event) {
        ...
        if ($button[0].id == "confirm-close") {
            ...
            var url = "{{url('admin/goods-receipt')}}";
            var table = $('#server-datatable').DataTable();
            table.ajax.url(url).load();
        }
        else
