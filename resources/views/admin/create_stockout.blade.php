@extends('layouts.vertical', ["page_title"=> "Stock Out Create"])
...
<div class="container-fluid">
                ...
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item">
                            <a href="{{url('admin/sales-invoice')}}">Stock Out</a></li>
    ...
    <div class="row">
        <div class="col-12">@if(!empty($errors->all()))
            ...
            @endif
            <div class="card">
                <form method="post" class="parsley-examples" action="{{url('admin/stockout/insert')}}">
                    {{csrf_field()}}
                        ...
                        <div class="row mb-3">
                            <div class="col-lg-4">
                                        ...
                                        <div class="form-group row">
                                                <label for="from_branch" class="col-sm-6 form-label">From Branch</label>
                                                <div class="col-sm-6"><input   type="text" class="form-control" name="from_branch" id="from_branch"  aria-describedby="basic-addon1" value="{{session('branch_name')}}" readonly>
                                        ...
                                        <div class="form-group row">
                                                ...
                                                <label for="to_branch" class="col-sm-6 form-label">To Branch <span class="text-danger">*</span></label>
                                                ...
                                                <input   type="hidden" class="form-control" name="to_branch_name" id="to_branch_name"  aria-describedby="basic-addon1"  readonly>
                                        ...
                                        <div class="form-group row">
                                                <label for="referral" class="col-sm-6 form-label">Address <span class="text-danger">*</span></label>
                                                <div class="col-sm-6"><textarea class="form-control" name="bill_to_address" id="bill_to_address" required></textarea>
                                        ...
                                        <div class="col-sm-12 mt-1">
                                            <a class="btn btn-primary waves-effect waves-light mb-0 qutt-bttn" id="copy_from_qutn" name="copy_from_qutn">Copy from Stock Transfer Request</a>
                                        </div>
                                        </div>
                            </div>
                            <div class="col-lg-4">
                                        ...
                                        <div class="form-group row">
                                                ...
                                                <div class="col-sm-6"><select name="status" class="statusSelect form-control select2" >
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group row">
                                    <label for="referral" class="col-sm-6 form-label">Total Quantity</label>
                                    <div class="col-sm-6"><input type="text" name="total_qty" id="total_qty" class="form-control" readonly>
                                </div>
                            </div>
                            <div class="form-group row">
...
document.getElementById('to_branch').addEventListener('change', function() {
    ...
    document.getElementById('to_branch_name').value = bname || '';
        ...
        $.ajax({
           type:'POST',
           url:"{{ url('admin/code') }}",
           data:{_token: "{{ csrf_token() }}",type:'stock-out'},
           success:function(data){
                            ...
                            +'<div class="echtr-inn">'
                                +'<div class="row">'
                                    ...
                                    +'<div class="colbor col-xl-2 col-lg-2 col-md-2 col-sm-4 col-6">'
                                        ...
                                        +'<div class="ech-td">'
                                            +'<label class="td-label">WHSE <span class="text-danger">*</span></label>'
                                            +'<div class="td-value"><select id="whscode_'+val+'"  name="whscode[]" class="form-control whscode-select" required></select><input type="hidden" id="unitprice_'+val+'" class="unitprice unitprice_'+val+' form-control" name="unitprice[]" readonly=""><input type="hidden" id="avqty_'+val+'" class="avqty avqty_'+val+' form-control" name="avqty[]" readonly=""></div></div></div>'
    ...
    function price_calc(r) 
    {
        ...
        var avl_quantity = (av_quantity==0) ? avqty : av_quantity;
        // Get the product name
        var productName = $("#product_" + r + " option:selected").text();

        // Check if entered quantity exceeds available quantity
        if (parseInt(quantity) > avl_quantity) {
            alert("The entered quantity for " + productName + " exceeds the available quantity (" + avl_quantity + ").");
            // Optionally, reset the quantity to the available quantity
            $("#quantity_" + r).val(avl_quantity);
        }
...
$('#to_branch').change(function() {
    ...
    if (selectedValue) {
        $.ajax({
            url: "{{ url('admin/stockout/ajax/stock_details') }}",
            type: 'GET',
            data: { branch_code: selectedValue },
            success: function(response) {
            var branch = response.branch;
            var stocktransfer = response.stocktransfer;
                // Assuming the response contains the necessary properties
                if (branch!=null && stocktransfer!=null) {
                    console.log(response.branch);
                    var stocktransfer = response.stocktransfer;
                    
                    $('#remarks').val(response.branch.remarks);
                    $('#partner3').empty();
                    $('#partner3').append(`<option value="${response.branch.emp_id}">${response.branch.name}</option>`);
                    $('#partner3').select2();
        ...
        $("#copy_from_qutn").click(function(e) {
            var selectedValue = $('#to_branch').val();

            
            e.preventDefault(); // Prevent default anchor behavior

            $.ajax({
                url: "{{ url('admin/stockout/ajax/branch_open_stock_details') }}",
                type: 'GET',
                data: { branch_code: selectedValue },
                ...
                success: function(data) { // Change 'quotation' to 'data' here
                    ...
                    if (data.length > 0) {
                        $.each(data, function(index, stock_transfer) {
                            $('#open_qutn').append(
                                $('<option></option>').val(stock_transfer.id).text(stock_transfer.doc_number)
                            );
                        });
                    } else {
  ...
  $('#open_qutn').change(function() {
    ...
    if (selectedValue) {
        $.ajax({
            url: "{{ route('admin.ajax.stockout_details') }}", // Update with your endpoint
            type: 'GET',
            data: { stock_transfer: selectedValue },
            success: function(response) {
              //  console.log(response.branch);
                    ...
                    $.each(response.data, function(index, item) {
                        ...
                        var rowHtml = `
                            <div class="ech-tr" id="tr_${i}">
                                        ...
                                        <div class="colbor col-xl-2 col-lg-2 col-md-2 col-sm-4 col-6">
                                            <div class="ech-td">
                                                ...
                                                <div class="td-value">
                                                    ...
                                                    <input type="hidden" id="avqty_${i}" required class="avqty_${i} form-control" value="${item.OnHand}"  name="avqty[]">
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div class="actn-td">
                                    <a href="javascript:void(0);" class="action-icon add-item"></a>
                                    <a href="javascript:void(0);" class="action-icon delete-item1 remove-row" data-value="${i}" value="${item.itemid}"></a>
                                </div>
                            </div>`;
    ...
    inputTimeout = setTimeout(function() {
        ...
        if (inputValue) {
            ...
            $.ajax({
                ...
                url: "{{ url('admin/stock-out/get_bproduct_details') }}",
...
$(document).on('click', '.remove-row', function(e) {
    ...
    // AJAX call for closing the Stock Out item and inserting data into the database
    $.ajax({
        url: "{{ url('admin/ajax/stocktransfer_close_items') }}",
        type: 'POST',
        data: {  
            _token: "{{ csrf_token() }}",
            item: itemValue,
